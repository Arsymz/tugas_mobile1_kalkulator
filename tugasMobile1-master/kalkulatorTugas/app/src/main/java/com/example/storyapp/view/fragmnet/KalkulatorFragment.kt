...
class KalkulatorFragment : Fragment() {
    ...
    private lateinit var buttonpersen:Button
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        buttonpersen=view.findViewById(R.id.btn_modulo)
        ...
        buttonpersen.setOnClickListener { appendToInput("%") }
    ...
    private fun calculateResult() {
        ...
        if (input.isNotEmpty()) {
            try {
                ...
                val fullexpression="$input=$result"
              hasil.text = fullexpression
            } catch (e: Exception) {
                hasil.text = "error"
            }
        }
    ...
    private fun eval(input: String): Number {
        return when {
            ...
            input.contains("%") -> {
                val parts = input.split("%")
                (parts[0].toDouble() / 100)  * parts[1].toDouble()
            }
            else -> input.toDouble()
        }
    }
